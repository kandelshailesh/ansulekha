...
let dataBuffer = fs.readFileSync('public/Nepal1.pdf');
let dataBuffer1 = fs.readFileSync('public/Nepal1.pdf');

// let dataBuffer = fs.readFileSync('public/Nepal2.pdf');
...
pdf(dataBuffer).then(function(data) {
    ...
    file1text=data.text.trim().toLowerCase();
  ...
  // PDF text
  console.log(data.text); 
  file2text=data.text.trim().toLowerCase();
...
// const pyprog = spawn('python', ['C:/Users/dell/Desktop/check.py',"Shailesh"],);
...
app.post('/searchstock', function (req, res) {
  ...
  //   return new Promise((resolve, reject) => {
      if (Number(req.body.index) === 0) {
        ...
        var getitemlatest=`SELECT sum(p.quantity) as totalpurchase,sum(s.quantity) as totalsales,i.itemname,ig.itemgroupname,ic.categoryname from itemtable as i inner join itemgroupnametable as ig on ig.itemgroupname=i.itemgroup inner join groupcategory as ic on ic.categoryname=ig.groupcategory inner join purchasetable as p on p.itemname= i.itemname inner join salestable as s on i.itemname=s.itemname group by i.itemname`;
        ...
        con.query(getitemlatest, function (err, result, fields) {
          if (err) {
            console.log(err);
        ...
        var getitemlatest=`SELECT sum(p.quantity) as totalpurchase,sum(s.quantity) as totalsales,i.itemname,ig.itemgroupname,ic.categoryname from itemtable as i inner join itemgroupnametable as ig on ig.itemgroupname=i.itemgroup inner join groupcategory as ic on ic.categoryname=ig.groupcategory inner join purchasetable as p on p.itemname= i.itemname inner join salestable as s on i.itemname=s.itemname where i.itemname= ? group by i.itemname`;

        var getgrouplatest=`SELECT sum(p.quantity) as totalpurchase,sum(s.quantity) as totalsales,i.itemname,ig.itemgroupname,ic.categoryname from itemtable as i inner join itemgroupnametable as ig on ig.itemgroupname=i.itemgroup inner join groupcategory as ic on ic.categoryname=ig.groupcategory inner join purchasetable as p on p.itemname= i.itemname inner join salestable as s on i.itemname=s.itemname where i.itemgroup= ? group by i.itemname`;

        var getcategorylatest=`SELECT sum(p.quantity) as totalpurchase,sum(s.quantity) as totalsales,i.itemname,ig.itemgroupname,ic.categoryname from itemtable as i inner join itemgroupnametable as ig on ig.itemgroupname=i.itemgroup inner join groupcategory as ic on ic.categoryname=ig.groupcategory inner join purchasetable as p on p.itemname= i.itemname inner join salestable as s on i.itemname=s.itemname where ig.groupcategory= ? group by i.itemname`;


        var returnquerylist = (index) => {
          var stocktablelist = {
            1:getcategorylatest,
            2:getgrouplatest,
            3:getitemlatest
        ...
        var indexfinal = returnquerylist(req.body.index);
        ...
        con.query(indexfinal,[[senditem]], function (err, result, fields) {
...
app.post('/filterstock', function (req, res) {
  console.log(req.body);
  ...
  //   return new Promise((resolve, reject) => {
      if (Number(req.body.index) === 0) {
        ...
        var getitemlatest=`SELECT sum(p.quantity) as totalpurchase,sum(s.quantity) as totalsales,i.itemname,ig.itemgroupname,ic.categoryname from itemtable as i inner join itemgroupnametable as ig on ig.itemgroupname=i.itemgroup inner join groupcategory as ic on ic.categoryname=ig.groupcategory inner join purchasetable as p on p.itemname= i.itemname inner join salestable as s on i.itemname=s.itemname where p.purchasedate between ? and ? and s.salesdate between ? and ? group by i.itemname`;

        // var gettotalpurchase = `SELECT sum(quantity) as totalpurchase from purchasetable where itemname in (SELECT itemliststock.itemname FROM itemliststock inner join itemtable on itemliststock.itemname=itemtable.itemname inner join itemgroupnametable on itemtable.itemgroup=itemgroupnametable.itemgroupname)`;
        con.query(getitemlatest,[startdate,enddate,startdate,enddate], function (err, result, fields) {
          if (err) {
            console.log(err);
            res.status(500).send({'err':'No any item in the list'});
            // reject(new Error("No any item found"));
        ...
        var getitemlatest=`SELECT sum(p.quantity) as totalpurchase,sum(s.quantity) as totalsales,i.itemname,ig.itemgroupname,ic.categoryname from itemtable as i inner join itemgroupnametable as ig on ig.itemgroupname=i.itemgroup inner join groupcategory as ic on ic.categoryname=ig.groupcategory inner join purchasetable as p on p.itemname= i.itemname inner join salestable as s on i.itemname=s.itemname where i.itemname= ? and  p.purchasedate between ? and ? and s.salesdate between ? and ?group by i.itemname`;

        var getgrouplatest=`SELECT sum(p.quantity) as totalpurchase,sum(s.quantity) as totalsales,i.itemname,ig.itemgroupname,ic.categoryname from itemtable as i inner join itemgroupnametable as ig on ig.itemgroupname=i.itemgroup inner join groupcategory as ic on ic.categoryname=ig.groupcategory inner join purchasetable as p on p.itemname= i.itemname inner join salestable as s on i.itemname=s.itemname where i.itemgroup= ? and p.purchasedate between ? and ? and s.salesdate between ? and ? group by i.itemname`;

        var getcategorylatest=`SELECT sum(p.quantity) as totalpurchase,sum(s.quantity) as totalsales,i.itemname,ig.itemgroupname,ic.categoryname from itemtable as i inner join itemgroupnametable as ig on ig.itemgroupname=i.itemgroup inner join groupcategory as ic on ic.categoryname=ig.groupcategory inner join purchasetable as p on p.itemname= i.itemname inner join salestable as s on i.itemname=s.itemname where ig.groupcategory= ? and p.purchasedate between ? and ? and s.salesdate between ? and ? group by i.itemname`;
        ...
        con.query(indexfinal,[senditem,startdate,enddate,startdate,enddate], function (err, result, fields) {
...
app.get('/daybook', function (req, res) {
  res.render('pages/daybook1');
})
app.post('/daybook', function (req, res) {
  var date1 = req.body.initialdate;
  var date2 = req.body.finaldate;
  console.log(date1);
  console.log(date2);
  var accountquery = "SELECT id,dctype,account,dcamount,vouchertype,transactionno,DATE_FORMAT(paymentdate,'%Y-%m-%d') as paymentdate FROM paymenttable where paymentdate between (?) and (?);SELECT `purchaseno`, `dctype`, `vouchertype`, `accountname`, `totalamount`, DATE_FORMAT(`purchasedate`,'%Y-%m-%d') as purchasedate FROM `purchasedetails` where purchasedate between (?) and (?) ; SELECT `salesno`, `dctype`, `vouchertype`, `accountname`, `totalamount`, DATE_FORMAT( `salesdate`,'%Y-%m-%d') as salesdate FROM `salesdetails`  where salesdate between (?) and (?)";
  con.query(accountquery, [date1, date2, date1, date2, date1, date2], function (err, result) {
    console.log(result);
    if (err) {
      console.log(err);
      res.status(500).send({
      ...
      res.status(200).send({
        'daybookprj': result[0],
        'daybookpurchase': result[1],
        'daybooksales': result[2]
      });
    }
  })
})
...
app.get('/trialbalance', function (req, res) {
  var accountlist = "SELECT * from accountinformation;"
  con.query(accountlist, function (err, results) {

    res.render('pages/trialbalance1', {
      accountnamelist: results
    });
  })
})

app.get('/trialbalance1', authenticate, (req, res) => {
  var accountquery = "SELECT SUM(dcamount),account from paymenttable where account in (SELECT accountname from accountinformation) group by account;";
...
app.post('/purchasesubmit', (req, res) => {
  ...
  var inser = "INSERT INTO `purchasetable`(`itemname`, `quantity`, `rate`, `units`, `amount`,`discountpercent`,`vatpercent`, `purchaseno`, `purchasedate`) VALUES ? ";
  ...
  var insertpurchasedetails = () => {
    return new Promise((resolve, reject) => {
      con.query(inser2,[[purchasedetailsdata[0],purchasedetailsdata[1],purchasedetailsdata[2]]], function (err, result) {
        if (result === undefined) {
          reject(new Error(err));
  ...
  insertpurchasedetails().then(function (results) {
    return new Promise((resolve, reject) => {
      ...
      for (i = 0; i < purchasetabledata[0]['itemname'].length; i++) {
        con.query(inser, [
          [
            [purchasetabledata[0]['itemname'][i], purchasetabledata[0]['quantity'][i], purchasetabledata[0]['rate'][i], purchasetabledata[0]['units'][i], purchasetabledata[0]['amount'][i],  purchasetabledata[0]['discountpercent'][i], purchasetabledata[0]['vatpercent'][i],purchasetabledata[0]['transactionno'][i], purchasetabledata[0]['purchasedate'][i]]
          ]
        ], function (err, result) {
          // console.log([paymentdata]);
          if (err) {
...
app.post('/salessubmit', (req, res) => {
  ...
  var inser = "INSERT INTO `salestable`(`itemname`, `quantity`, `rate`, `units`, `amount`, `salesno`,`discountpercent`,`vatpercent`, `salesdate`) VALUES ? ";
  ...
  var insertsalesdetails = () => {
    return new Promise((resolve, reject) => {
      con.query(inser2, [[salesdetailsdata[0],salesdetailsdata[1],salesdetailsdata[2]]], function (err, result) {
        if (result === undefined) {
          console.log(err);
          reject(new Error(err));
  ...
  insertsalesdetails().then(function (results) {
    ...
    for (i = 0; i < salestabledata[0]['itemname'].length; i++) {
      con.query(inser, [
        [
          [salestabledata[0]['itemname'][i], salestabledata[0]['quantity'][i], salestabledata[0]['rate'][i], salestabledata[0]['units'][i], salestabledata[0]['amount'][i], salestabledata[0]['transactionno'][i],salestabledata[0]['discountpercent'][i],salestabledata[0]['vatpercent'][i], salestabledata[0]['salesdate'][i]]
        ]
      ], function (err, result) {
        // console.log([paymentdata]);
        if (err) {
          console.log(err);
          res.status(500).send("Transaction didnot completed");
...
app.get('/saveusergroup', function (req, res) {
  var usergroupname = req.body.usergroup;
...
app.post('/trialbalance', function (req, res) {
  console.log(req.body);
  var getledgerdata1 = `SELECT DATE_FORMAT(paymentdate,'%Y-%m-%d') as date,vouchertype,dctype,dcamount,account as accountname, transactionno from paymenttable where transactionno in (SELECT DISTINCT paymenttable.transactionno from paymentdetails inner join paymenttable on paymentdetails.transactionno=paymenttable.transactionno where paymenttable.account=?) and vouchertype in (SELECT DISTINCT paymenttable.vouchertype from paymentdetails inner join paymenttable on paymentdetails.transactionno=paymenttable.transactionno where paymenttable.account=?)   and account!=?`;
  ...
  Promise.all([functiongetledgerdata1(), functiongetledgerdata2(), functiongetledgerdata3()]).then(function (result) {
    ...
    if (req.body.accountname !== 'Purchases Account') {
      console.log("True");
      for (i = 0; i < result[1].length; i++) {
        result[1][i].accountname = "Purchases Accounts"
      }
    }
    if (req.body.accountname !== 'Sales Account') {
      console.log("True");
...
app.get('/pdftester',function(req,res)
{
  res.render('pages/pradeepvaipdf');
})
